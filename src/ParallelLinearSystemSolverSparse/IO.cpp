...
{
    mat matrix = read_matrix(dimensions, filename);
    return SparseMatrix(matrix, non_zero_members_in_row);
}
...
{
    mat matrix = read_sub_matrix(dimensions, filename, rank, np);
    return SparseMatrix(matrix, non_zero_members_in_row);
}

void print(const SparseMatrix &A)
{
    print(A.ToDenseMatrix());
}
